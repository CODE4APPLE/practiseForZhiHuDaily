...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [[UINavigationBar appearance] setTitleTextAttributes: [NSDictionary dictionaryWithObjectsAndKeys:
                                                           [UIColor whiteColor], NSForegroundColorAttributeName, [UIFont boldSystemFontOfSize:14], NSFontAttributeName, nil]];
    [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleLightContent];

    return YES;
}

- (void)loadLaunchImage:(void(^)(UIImage*))completion {
    NSString *urlString = [NSString stringWithFormat:LAUNCH_IMAGE_URL,pixelSize];

    [HTTPHelper GETURL:urlString completionHandler:^(NSDictionary *result, NSError *error) {
        NSURL *imgURL = [NSURL URLWithString:[result objectForKey:@"img"]];
